          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {service.process.map((step, i) => (
              <motion.div
                ...
                className="p-6 bg-secondary rounded-xl relative"
              >
                <span className="text-5xl font-heading font-bold text-primary/20 absolute top-4 right-4">{step.step}</span>
        ...
        <div className="container mx-auto px-4 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {service.benefits.map((benefit) => (
              <div key={benefit} className="flex items-center gap-3 p-4 bg-card-gradient border border-border rounded-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {service.plans.map((plan, i) => (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: i * 0.1 }}
                className={`p-8 rounded-xl relative ${plan.popular ? "bg-glass border-2 border-primary shadow-glow" : "bg-secondary border border-border"}`}
              >
                {plan.popular && (
                ...
                <div className="text-3xl font-heading font-bold text-gradient mb-6">{plan.price}</div>
